...
void CuSuiteSummary(CuSuite *testSuite, CuString *summary) {
  ...
  for (i = 0; i < testSuite->count; ++i) {
    CuTest *testCase = testSuite->list[i];
    char name[1024];
    strcpy(name, testCase->failed ? "\033[1;31m" : "\033[1;32m");
    strcat(name, testCase->name);
    strcat(name, testCase->failed ? " failed!\n" : " success!\n");
    strcat(name, "\033[0m");
    CuStringAppend(summary, name);
  }
...
void CuSuiteDetails(CuSuite *testSuite, CuString *details) {
  ...
  if (testSuite->failCount == 0) {
    ...
    const char *testWord = passCount == 1 ? "case" : "cases";
    CuStringAppendFormat(details, "\033[1;32mSuccess! (Total %d %s)\033[0m\n\n",
                         passCount, testWord);
  } else {
    if (testSuite->failCount == 1)
      CuStringAppend(details, "\033[1;31mThere was 1 failure:\033[0m\n");
    else
      CuStringAppendFormat(details,
                           "\033[1;31mThere were %d failures:\033[0m\n",
                           testSuite->failCount);
    ...
    for (i = 0; i < testSuite->count; ++i) {
      ...
      if (testCase->failed) {
        failCount++;
        CuStringAppendFormat(details, "\033[1;31m%d) %s: %s\033[0m\n",
                             failCount, testCase->name, testCase->message);
      }
    }
    CuStringAppend(details, "\033[1;31m\n!!!FAILURES!!!\033[0m\n");

    CuStringAppendFormat(details, "Total %d runs: ", testSuite->count);
    CuStringAppendFormat(details, "\033[1;32m%d passes, \033[0m",
                         testSuite->count - testSuite->failCount);
    CuStringAppendFormat(details, "\033[1;31m%d fails\033[0m\n",
                         testSuite->failCount);
  }
}
